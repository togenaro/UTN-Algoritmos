...
void menuPyR(bool &bandera_sesion_p, bool &bandera_sesion_r, FILE *arch);
void moduloRecepcionistas(bool &bandera_sesion_p, bool &bandera_sesion_r, FILE *arch);
void moduloEspacios(bool &bandera_sesion_p, bool &bandera_sesion_r, FILE *arch);
...
{	
	...
	do
	{
		...
		switch(opcion)
		{
			case 1: { menuPyR(bandera_sesion_p, bandera_sesion_r, arch); break; }
			case 2: { menuAdministracion(arch); break; }
			case 3:
					{
						system("cls");
						printf("Finalizando...");
...
{	
	...
	do
	{
		...
		switch( opcion )
		{
			case 1: { moduloEspacios(bandera_sesion_p, bandera_sesion_r, arch);	break; }
			case 2: { moduloRecepcionistas(bandera_sesion_p, bandera_sesion_r, arch); break; }
...
{	
	if(bandera_sesion_p == false)
	{
		do 
		{
			...
			if(bandera_sesion_p == false)
				menuPyR(bandera_sesion_p, bandera_sesion_r, arch);	
		}	
	...
	do
	{
		...
		switch( opcion )
		{
			...
			case 5: { menuPyR(bandera_sesion_p, bandera_sesion_r, arch); }
...
void moduloRecepcionistas(bool &bandera_sesion_p, bool &bandera_sesion_r, FILE *arch)
{
	if(bandera_sesion_r == false)
	{
		do 
		{
			...
			if(bandera_sesion_r == false)
				menuPyR(bandera_sesion_r, bandera_sesion_r, arch);	
		}	
...
bool inicioSesion_P(FILE *arch)
{
	...
	else
	{
		...
		rewind(arch);
		fread(&sesion.usuario, sizeof(sesion.usuario), 1, arch);
		fread(&sesion.clave, sizeof(sesion.clave), 1, arch);
...
{	
	...
	else
	{
		...
		{
			system("cls");
			printf("Usuario y/o contraseña incorrecto/s. Ingrese nuevamente...");
			printf("\n\n");
			system("pause");
			system("cls");	
		}					
